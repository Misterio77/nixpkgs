...
in
{
  options.services.photoprism = with lib; {
    ...
    databaseDriver = mkOption {
      ...
      default = "sqlite";
      description = ''
        Database driver, select <literal>"sqlite"</literal> to use an embedded database.

        Photoprism recommends a MariaDB database (using the
    ...
    package = mkOption {
      ...
      defaultText = lib.literalExample "pkgs.photoprism";
